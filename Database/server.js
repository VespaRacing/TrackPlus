...
const corsOptions = {
  ...
  methods: ['GET', 'POST'],        // Allow methods you are using
  credentials: true,
};
...
passport.use(new GoogleStrategy({
    ...
    callbackURL: `/auth/google/callback`, // Dynamic callback URL using PORT
...
passport.deserializeUser((id, done) => {
    ...
    console.log("Deserializing user ID:", id); // Added log
   done(null, id);
});
...
app.post('/loginUser', (req, res, next) => {
    ...
    db.get('SELECT * FROM utenti WHERE id = ? AND pwd = ?', [username, password], (err, row) => {
	    ...
	    req.session.user = { id: row.id, username: row.username };
	     ...
	     // Manually set session data - NOT RECOMMENDED for authentication in real apps
            req.session.user = {
                id: row.id,
                role: row.role,
                // ... other user properties you want to store in the session
            };
            req.session.isLoggedIn = true; // Manually set a flag to indicate login
